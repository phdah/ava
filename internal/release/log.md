...
- **ChatGPT Work Cloud release qualification**: Moved all non-CI release qualification onto ChatGPT-hosted Work Cloud execution. Deterministic scenario setup and validation run in the Work cloud shell; semantic interactions run as fresh Work subagents against isolated cloud workspaces; a separate fresh Work subagent performs the independent audit. The canonical qualification entry point no longer invokes OpenCode or any user-local agent runtime, and compact evidence is bound to exact repository/release/fixture identities rather than product session IDs.
