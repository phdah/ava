...
**Resolve dogfood findings 28 and 29, then assemble a new exact corrective-alpha candidate and run fresh full qualification.**

Findings 22 through 27 are implementation-complete. Qualification run `20260824T122451984003Z-alpha14-to-alpha15-corrective-local` (candidate `77977f8`) passed all 17 runner scenarios and all repository tests but ended `needs-review`: the independent audit found two major issues and one minor issue in inbox ingestion. Finding 27 resolves the minor execution-scope issue. The next sequence is:

1. resolve [finding 28](todo/05-release-qualification/dogfood/28-require-reconciled-inbox-disposition-evidence.md) (bounded scope: require reconciled per-passage disposition evidence)
2. decide an approach for and resolve [finding 29](todo/05-release-qualification/dogfood/29-decide-runner-inbox-semantic-detection-approach.md) (no bounded scope yet; needs an explicit approach decision first)
3. assemble a new exact candidate from the updated corrective-alpha release PR revision
4. rerun the complete 17-scenario qualification matrix through the normal repository-owned OpenCode adapter and obtain explicit user signoff
...
[Finding 25](todo/05-release-qualification/dogfood/25-offer-qualification-failure-todo-tracking.md) is post-v1 and does not block release progression. Findings 28 and 29 are the only pending dogfood findings blocking the corrective alpha.
