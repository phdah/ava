...
5. [V1 release qualification](05-release-qualification/) - 3 of 6 core gates complete; dogfood active with 8 pending findings, 5 pending blockers, 2 pending dogfood `required-v1` findings, 3 pending supporting qualification tasks, and 28 completed findings
...
Alpha publication is complete through immutable `1.0.0-alpha.14`. Findings 22 through 29 are implementation-complete. Qualification run `20260824T122451984003Z-alpha14-to-alpha15-corrective-local` (candidate `77977f8`) passed all 17 runner scenarios and all repository tests but ended `needs-review` on independent-audit findings in inbox ingestion. Findings 27 through 29 resolve the execution-scope, rendered disposition-reconciliation, and runner semantic-claim issues from that audit. A 2026-08-24/25 investigation then found the qualification pipeline itself operationally unreliable (session-lifecycle fragility, no cross-run resume, no pollable status, and no sanctioned Office-document reader), recorded as findings 30-36; finding 33 (deciding whether to keep, shrink, or remove the multi-hour `complete-pending-inbox` scenario) is next up and upstream of findings 30, 31, 32, and 34, all pending blockers that preempt the corrective alpha. Findings 35 and 36 are pending `required-v1` findings that block the release-candidate gate. The remaining ordered path to the first stable release is:

Resolve dogfood finding 33, then findings 30, 31, 32, and 34, then assemble a new exact candidate from the updated corrective-alpha release PR revision and rerun the complete qualification matrix. A new blocker may still preempt that run without changing the six canonical release steps:
...
4. publish the release candidate only after dogfood closure and any newly discovered blockers or required RC work are resolved
...
Phase 7 investigates durable evidence for manually prompted semantic mutations after the Backlog.md task model is established. It is proposal-only until the user approves any public format, ownership, or role-authority change, and it does not block the v1 release unless explicitly reprioritized.
