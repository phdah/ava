...
5. [V1 release qualification](05-release-qualification/) - 3 of 6 core gates complete; dogfood active with 1 pending finding, 0 pending blockers, 0 pending dogfood `required-v1` findings, 3 pending supporting qualification tasks, and 24 completed findings
...
Alpha publication is complete through immutable `1.0.0-alpha.14`. Findings 22, 23, and 24 are implementation-complete. The remaining ordered path to the first stable release is:

The next qualification run must use a new exact candidate assembled from the updated corrective-alpha release PR revision and must exercise the repository-owned OpenCode large-JSON buffering without the former external shim. A new blocker may still preempt that run without changing the six canonical release steps:
