...
5. [V1 release qualification](05-release-qualification/) - 3 of 6 core gates complete; dogfood active with 4 pending findings, 3 pending blockers, 0 pending dogfood `required-v1` findings, 3 pending supporting qualification tasks, and 25 completed findings
...
Alpha publication is complete through immutable `1.0.0-alpha.14`. Findings 22 through 26 are implementation-complete. Qualification run `20260824T122451984003Z-alpha14-to-alpha15-corrective-local` (candidate `77977f8`) passed all 17 runner scenarios and all repository tests but ended `needs-review` on independent-audit findings in inbox ingestion. Findings 27, 28, and 29 record those findings and currently block the next prerelease; 27 and 28 have a bounded resolution scope, while 29 needs an explicit approach decision before implementation. The remaining ordered path to the first stable release is:

Resolve findings 27, 28, and 29, assemble a new exact candidate from the updated corrective-alpha release PR revision, and rerun the complete qualification matrix. A new blocker may still preempt that run without changing the six canonical release steps:
