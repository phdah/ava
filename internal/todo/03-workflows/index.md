# Phase 03: Workflow System

Define workflows as focused predefined prompts that activate exactly one primary role without duplicating durable role instructions.

## Tasks

1. [x] [Define the workflow format contract](01-define-workflow-format-contract.md)
2. [x] [Define the workflow registry and routing contract](02-define-workflow-registry-and-routing.md)
3. [x] [Create the initial built-in workflow catalog](03-create-built-in-workflow-catalog.md)
4. [ ] [Review workflow purpose and built-in catalog](04-review-workflow-purpose-and-built-in-catalog.md)
5. [ ] [Define workflow trigger portability](05-define-workflow-trigger-portability.md)
6. [ ] [Define workflow lifecycle ownership](06-define-workflow-lifecycle-ownership.md)

The remaining tasks are deferred until the distribution ownership, versioning, and migration contracts establish how workflows are classified, versioned, and migrated in installed projects. Resume this phase after the required Phase 04 design tasks are complete.

## Previous phase

[Core roles for initialized projects](../02-core-roles/).

## Next active phase

[Versioned distribution and upgrades](../04-distribution-and-upgrades/).