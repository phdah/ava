...
A two-day operational-reliability investigation (2026-08-24 to 2026-08-25) into the release qualification pipeline itself - `qualification_automation.py`, `qualification_runner.py`, the `qualify-release.sh`/`qualify-synthetic.sh` flow, and the ad hoc operator monitoring pattern used around them - recorded findings 30-36. Findings 30 and 33 are complete; findings 31-32 and 34-36 remain pending.

**Finding 31 is next up.** Finding 30 was closed as a no-op after reassessment showed that operator-session death had not been established as the actual cause of the failed run, so no detached launcher or release-tooling change was retained. Resolve findings 31, 32, and 34 before rerunning full qualification.
