...
A two-day operational-reliability investigation (2026-08-24 to 2026-08-25) into the release qualification pipeline itself - `qualification_automation.py`, `qualification_runner.py`, the `qualify-release.sh`/`qualify-synthetic.sh` flow, and the ad hoc operator monitoring pattern used around them - found that the pipeline was largely unworkable in practice over two consecutive days of attempts, independent of any content/fidelity finding. Findings 30-36 record the resulting operational-reliability findings. Findings 30 and 33 are complete; findings 31-32 and 34-36 remain pending.

**Finding 31 is next up.** Finding 30 now provides a sanctioned `nohup`/`setsid` detached qualification launcher with an external log and a real SIGHUP process-group regression, so the operator session no longer owns the qualification process tree. Resolve findings 31, 32, and 34 before rerunning full qualification.
...
[Detach qualification automation's process tree from the operator session lifecycle](30-detach-qualification-process-tree-from-session.md) is complete. Full qualification now launches through a repository-owned detached wrapper that survives parent-shell SIGHUP, records its PID and external log, and groups the raw evidence root under the same repository-external launch root.
...
[Add a sanctioned deterministic Office-document reader tool for inbox ingestion](34-add-sanctioned-office-document-reader-tool.md) is pending. Finding 27's ad hoc code ban also removed the only mechanism for reading `.docx`/`.pptx` sources. Finding 33 intentionally retains one source of each format, including one `.docx` and one `.pptx`, so `complete-pending-inbox` still cannot complete until a sanctioned reader exists.
...
[Prohibit ad hoc code execution during inbox ingestion](27-prohibit-ad-hoc-code-during-inbox-ingestion.md) is complete. Inbox Ingester now explicitly prohibits creating or executing ad hoc code and programmatic bulk-content transformers as an ingestion mechanism, and the complete-inbox runner guards against transient direct project-root helper artifacts.
...
| 26 | completed | blocker | next prerelease | [Remove the hardcoded semantic-inspection-path qualification gate](26-remove-hardcoded-semantic-inspection-path-gate.md) |
