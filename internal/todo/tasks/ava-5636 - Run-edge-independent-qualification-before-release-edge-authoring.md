...
The maintained 17-scenario matrix declares one explicit phase per scenario: 12 edge-independent scenarios covering target install/routing/inbox/damage/lifecycle behavior, and 5 edge-dependent scenarios covering authentic upgrade resume, abort, rollback, finalization, and semantic reconciliation.
...
`qualify-release.sh` is the qualification execution entry point. Phase orchestration and phase selection are separate internally, with `qualification_runner.py` providing the shared scenario engine and `qualification_automation.py` providing shared automation helpers.
