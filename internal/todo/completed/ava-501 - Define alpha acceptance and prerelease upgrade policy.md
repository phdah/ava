...
Define the exact readiness gate, test scope, defect policy, support boundary, and upgrade declarations required before the first Ava alpha. This task also preserves the Phase 05 roadmap-level context that previously lived separately.
...
- [Alpha qualification policy](/internal/release/alpha-qualification.md) defines the two-state gate, required evidence, defect classes, protected impacts, prerelease support boundary, and approval scope.
- [Alpha qualification fixture](/internal/release/fixtures/alpha-qualification.json) freezes the machine-readable gate and intended source-to-target transitions.
- [Alpha qualification tests](/internal/release/tests/test_alpha_qualification.py) validate Phase 1 through Phase 4 completion, evidence references, reproducible assembly, the first-alpha empty upgrade-edge list, explicit later prerelease edges, refusal of unversioned sources, and revision-bound approval.
- [Release publication procedure](/internal/release/procedure.md) now requires the applicable qualification gate before publication.
...
## Migrated Phase 05 roadmap context

Phase 05 turns Ava's format, roles, release tooling, OpenCode support, and conformance suite into tested prereleases and the first stable distribution.

Core release gates: alpha policy, release-please integration, and `1.0.0-alpha.1` publication are complete. Alpha dogfooding, release-candidate publication, and stable publication remain unfinished. Supporting qualification work includes the synthetic qualification vault, corrective alpha, automated qualification/evidence state, qualification permission hardening, qualification session isolation, and RC stabilization.

Further alpha dogfooding and immediate V1 progression are parked by explicit user decision. The dogfood umbrella remains pending but is not the active work queue. The qualification OpenCode permission and session-inventory defects discovered during `v1.0.0-alpha.15` are implementation-complete. Roadmap execution moved to Backlog.md integration and then durable interaction evidence before reassessment with the user.

The parked conceptual path to `1.0.0` remains: finish trustworthy qualification evidence, complete corrective-alpha evidence obligations, obtain explicit user closure of dogfooding, publish the RC, stabilize the RC, then qualify and publish stable `1.0.0`. This sequence is not active unless the user explicitly resumes release progression.