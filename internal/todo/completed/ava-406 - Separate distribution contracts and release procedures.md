...
Separate public distribution contracts, release payload sources, and maintainer-only publication procedures. The complete pre-Backlog task record is preserved below.
...
`internal/release/validate-boundaries.sh` checks required public contracts and schemas, absence of obsolete template contract locations, expected `/templates/` children, canonical schema identifiers, stale references, and release-source independence from internal repository content.

The next work at the time was workflow trigger portability and lifecycle ownership, followed by installer/updater work after Phase 3.